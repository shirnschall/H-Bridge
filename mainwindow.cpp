...
{
    //get filename
    QString filename=QFileDialog::getSaveFileName(this, tr("Save As"), "", "C (*.c);;Header (*.h)");
    if(filename!="")
    {
...
{
    //get filename
    QString filename=QFileDialog::getOpenFileName(this, tr("Save As"), "", "C (*.c);;Header (*.h)");
    std::cout<<filename.toStdString().c_str()<<std::endl;
    bool open{true};
...
//    {
//        if(ui->listWidget->item(i)->text().endsWith('*'))
//        {
//            saved=false;
//            break;
//        }
...
//        event->accept();
    ...
    QMessageBox::StandardButton resBtn = QMessageBox::question( this, "HBridge",
                                                                tr("<font color=\"#000\">Save open documents?</font>"),
                                                                QMessageBox::Cancel | QMessageBox::No | QMessageBox::Yes,
                                                                QMessageBox::Yes);
    if (resBtn == QMessageBox::Yes)
    {
        //t1.~thread();
        MainWindow::on_actionSave_All_triggered();
        event->accept();
    }
...
void MainWindow::on_actionSave_All_triggered()
{
    for(int i=0;i<ui->listWidget->count();++i)
    {
        CodeEditor *current =dynamic_cast<CodeEditor *>(ui->stackedWidget->widget(i));

        if(locations.at(i)!="")
        {
            ofstream file;
            file.open(locations.at(i).c_str());
            if(current)
            {
                file << current->toPlainText().toStdString().c_str();
            }
        ...
        else if(locations.at(i)=="" && current->toPlainText() == "#include <avr/io.h>\n#include <stdint.h>\n\n#define F_CPU 8000000UL\n\n\n\nint main(void) {\n    \n    DDRB  = 0xFF;\n    PORTB = 0x03;\n    \n    \n    \n    return 0;\n}")
        {
